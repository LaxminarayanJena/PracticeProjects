...
public class CV_NB extends PDFTextStripper {
   ...
   // WebDriver driver;
    ...
    public   void test(@Optional("4") int iterationNumber) throws InterruptedException, Exception {
...
//        *******************Adding Driver Details*******************
        ...
        if (inputData.getColumnData("state").equalsIgnoreCase("CA")) {
            WebElement lossesDropdown = driver.findElement(By.xpath(OR.getObjectProperty("losses")));
            Select losses = new Select(lossesDropdown);
            losses.selectByValue(inputData.getColumnData("driverLosses"));
        } else if (inputData.getColumnData("state").equalsIgnoreCase("PA") || inputData.getColumnData("state").equalsIgnoreCase("KY")) {
            WebElement lossesDropdown = driver.findElement(By.xpath(OR.getObjectProperty("lossesPA")));
            Select losses = new Select(lossesDropdown);
            losses.selectByValue(inputData.getColumnData("driverLosses"));
        } else if (inputData.getColumnData("state").equalsIgnoreCase("NC")) {
            WebElement lossesDropdown = driver.findElement(By.xpath(OR.getObjectProperty("lossesNC")));
            Select losses = new Select(lossesDropdown);
            losses.selectByValue(inputData.getColumnData("driverLosses"));
            WebElement yearLicensedDropdown = driver.findElement(By.xpath(OR.getObjectProperty("yearsLicensed")));
            Select yearLicensed = new Select(yearLicensedDropdown);
            yearLicensed.selectByValue(inputData.getColumnData("yearLicensed"));
        } else {
        }
        if (inputData.getColumnData("state").equalsIgnoreCase("CA")) {
...
//      *******************Adding vehicle 2*******************
        ...
        vehicle2ModificationType.selectByIndex(0);
...
//       *******************Adding Vehicle2 Questions*******************
     ...
     //   *******************Adding vehicle 3*******************
        ...
        vehicle3ModificationType.selectByIndex(0);
