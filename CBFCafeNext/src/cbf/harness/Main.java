/******************************************************************************
$Id : Main.java 3/3/2016 7:07:22 PM
...
public class Main {
	...
	public static void main(String args[]) {
		...
		System.out.println(args+""+runMap+"-------------------------------------------");
	...
	private static Map<String, String> parseCommandLineArgs(String[] args) {
		...
		options.addOption(OptionBuilder.withArgName("jsoninputfile").hasArg()
				.create("jsoninputfile"));
		...
		options.addOption(OptionBuilder.withArgName("almTestSet").hasArg()
				.create("almTestSet"));
		options.addOption(OptionBuilder.withArgName("browser").hasArg().create(
				"browser"));
		...
		try {
			...
			if (!commandLine.hasOption("configfilepath")) {
				logger
						.handleError("Parameter 'configfilepath' must be defined");
			}
