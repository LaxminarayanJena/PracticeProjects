/******************************************************************************
$Id : ExcelAccess.java 3/3/2016 7:07:32 PM
		...
		public String[] get() {
			...
			for (int j = 0; j < cells.size(); j++) {
				...
				case Cell.CELL_TYPE_FORMULA:
					if (DateUtil.isCellDateFormatted(cells.get(j))) {
						colValues[j] = new SimpleDateFormat("MM/dd/yyyy")
								.format(cells.get(j).getDateCellValue());
					} else {
						colValues[j] = new DataFormatter()
								.formatCellValue(cells.get(j));
					}
					break;
